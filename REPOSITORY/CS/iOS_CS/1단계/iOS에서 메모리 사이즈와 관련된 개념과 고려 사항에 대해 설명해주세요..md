...
#### 메모리 워드 크기
- CPU가 한번에 처리하는 데이터 크기
- 2013 이후 MAC은 64bit 
#### 데이터 정렬
- 메모리에서 데이터가 정렬되고 액세스되는 방식
- **데이터 정렬, 데이터 구조 패딩(padding), 패킹(packing)** 과 같은 세 가지 개별적인 관련 문제로 구성
- 최신 CPU는 데이터가 자연스럽게 정렬 _(naturally aligned)_ 될 때(일반적으로 데이터 주소가 데이터 사이즈의 배수일 때) 메모리에 대한 읽기 및 쓰기를 가장 효율적으로 수행할 수 있음
- 데이터 얼라인먼트는 정렬되는 요소가 _자연 정렬(natural alignment)_ 을 따르는 것을 의미..자연 정렬을 보장하기 위해 구조 요소(structure elemetns) 사이에 또는 구조의 마지막 요소(the last element of a structure) 뒤에 패딩(padding)을 삽입해야함
### 💡포인터 크기(32비트, 64비트)에 따른 메모리 사용량 차이와 고려 사항에 대해 설명해주세요.
#### 포인터 크기
포인터의 크기는 운영체제의 bit수에 따라 달라짐
- 32bit 시스템의 포인터 크기: 4 byte
- 64bit 시스템의 포인터 크기: 8 byte
#### 포인터 크기 원리
- 포인터 = 메모리 위치를 가리키기 위한 주소를 담는 변수
- 32bit 라면 cpu의 메모리 접근을 위한 주소가 32비트만큼 가능하기 때문에 포인터도 32bit(4바이트)가 됨
- 반면 64bit 라면 cpu의 메모리 접근을 위한 주소가 64bit만큼 가능하기 때문에 포인터도 64bit(8byte)가 됨
...
- [메모리 얼라인먼트](https://minusi.tistory.com/entry/%EB%A9%94%EB%AA%A8%EB%A6%AC-%EC%96%BC%EB%9D%BC%EC%9D%B8%EB%A8%BC%ED%8A%B8Memory-Alignment)